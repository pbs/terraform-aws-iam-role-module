...
```hcl
module "iam_role" {
    source = "github.com/pbs/terraform-aws-iam-role-module?ref=0.0.1"
}
```
...
gh -R 'pbs/terraform-aws-iam-role-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/iam-role'
rm -f release.tar.gz
```
...
```bash
git remote add -f iam-role-module git@github.com:pbs/terraform-aws-iam-role-module.git
git subtree add --prefix terraform/modules/iam-role iam-role-module main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/iam-role iam-role-module $REF --squash
```
